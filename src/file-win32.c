...
ssize_t hnd_read(HANDLE hnd, int* restrict pinfo, void* buf, size_t nbyte)
{
	...
	// FD_FLAG_HOLD_CR implies FD_FLAG_TEXT
	if (fd_info & FD_FLAG_HOLD_CR) {
		*pinfo &= ~FD_FLAG_HOLD_CR;
		// CR is held, hence we need to read less to accomodate with
		// the potential extra character to write to output buf.
		if (nbyte-- <= 1)
			buf = tmp;
	}
