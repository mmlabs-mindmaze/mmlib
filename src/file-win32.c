...
int copy_internal(const char* src, const char* dst, int flags, int mode)
{
	...
	bool nofollow = flags & MM_NOFOLLOW;

	attrs = FILE_ATTRIBUTE_NORMAL;
	attrs |= nofollow ? FILE_FLAG_OPEN_REPARSE_POINT : 0;
	hnd_src = open_handle(src, GENERIC_READ, OPEN_EXISTING, NULL, attrs);
	if (hnd_src == INVALID_HANDLE_VALUE) {
		// Force to open handle to identify EISDIR error case
		if (GetLastError() == ERROR_ACCESS_DENIED) {
			hnd_src = open_handle_for_metadata(src, nofollow);
			if (hnd_src != INVALID_HANDLE_VALUE)
				goto testinfo;
		}
