...
START_TEST(copy_symlink)
{
	...
	ck_assert(mm_copy(LINKNAME, TEST_FILE, 0) == 0);
	...
	// Make symlink copy
	ck_assert(mm_copy(LINKNAME, TEST_FILE, MM_NOFOLLOW) == 0);
	ck_assert(are_files_same(afile, TEST_FILE) == true);
...
START_TEST(copy_fail)
{
	...
	mm_copy(afile, TEST_FILE, 0);
	ck_assert(mm_copy(afile, TEST_FILE, 0) == -1);
	ck_assert_int_eq(mm_get_lasterror_number(), EEXIST);
	mm_unlink(TEST_FILE);

	// test regular file copy does not overwrite file
	mm_copy(LINKNAME, TEST_FILE, 0);
	ck_assert(mm_copy(LINKNAME, TEST_FILE, MM_NOFOLLOW) == -1);
	ck_assert_int_eq(mm_get_lasterror_number(), EEXIST);
	mm_unlink(TEST_FILE);
